...
export default function Navbar({ images, title, texts }: NavbarProps) {
  const [isOpen, setIsOpen] = useState(false)

  return (
    <nav className="relative w-full z-10 text-white mx-auto">
      <div className="absolute top-0 left-0 right-0 z-10 h-30 bg-black bg-opacity-70" 
           style={{ maxWidth: '1300px', height:'130px', marginTop: '50px', marginLeft: 'auto', marginRight: 'auto' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center h-full">
          <div className="flex-shrink-0 flex items-center">
            <Link href="/" className="flex items-center">
              <Image src="/logo.png" alt="빛 축제 로고" width={100} height={100} className="mr-2" />
              <span className="text-2xl font-bold">빛 축제</span>
          </Link>
          </div>
      ...
      <div className="relative w-full h-[550px] z-0 bg-black opacity-40">
        {images.length > 1 ? (
          ...
          <div className="relative w-full h-full">
            ...
            />
            {/* Single Image Text */}
            <div className="absolute inset-0 flex justify-center items-end pb-36">
              <h2 className="text-6xl font-semibold text-white">{texts[0]}</h2>
            </div>
          </div>
        )}
      </div>
